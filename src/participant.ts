...
import { show, update, checkIn, undoCheckIn, destroy } from './adapter/participants';
...
export default class Participant extends ChallongeBase {
  ...
  public checkIn() {
    return new Promise((resolve, reject) => {
      checkIn(this.api_key, this.baseUrl, this.id).then(res => {
        Object.assign(this, res.participant);
        resolve(this);
      }).catch(err => reject(err));
    });
  ...
  public undoCheckIn() {
    return new Promise((resolve, reject) => {
      undoCheckIn(this.api_key, this.baseUrl, this.id).then(res => {
        Object.assign(this, res.participant);
        resolve(this);
      }).catch(err => reject(err));
    });
  ...
  public destroy() {
    return new Promise((resolve, reject) => {
      destroy(this.api_key, this.baseUrl, this.id).then(res => {
